...
R1.geometry('712x712')
R1.title('driver drowsiness detection system')
...
'''def head(): 
    ...
    while True:    
        ...
        if y_movement > gesture_threshold:
            gesture = 'bhavioural analysis Detected'
...
'''def ped():
    ...
    # Open the video
    cap = cv2.VideoCapture(0)  #"videoplayback.mp4"
    while True:
        ret, frame = cap.read()
        if ret == False:
            break
...
#loginbt1 = Button(R1,text = "pedestrian detection",width=15,height=2,bg="lightblue",fg="white",font="5",relief=RAISED,overrelief=RIDGE,command=ped)
#signUpbt = Button(R1,text = "head bend",width=10,height=2,bg="lightblue",fg="black",font="5",relief=RAISED,overrelief=RIDGE,command=head)
